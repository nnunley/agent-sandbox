...
| SCENARIO-0117 | Host-signed laneq RPC accepted | integration | iteration | **laneq:** `cd /Users/ndn/development/laneq && uv run pytest tests/test_grpc_auth.py -k passes_through` + Go `go test ./grantauth/` (interop: Go-signed grant+proof verify in pyseto, proven manually). Live e2e (run-laneq-wire.sh + enforce) → pending | STORY-0080 |
| SCENARIO-0118 | Forged / expired / replayed grant or proof rejected | integration | iteration | **laneq:** `cd /Users/ndn/development/laneq && uv run pytest tests/test_auth.py tests/test_proof.py tests/test_grpc_auth.py` (forged/expired/wrong-aud grant; forged/wrong-method/stale/replayed proof → UNAUTHENTICATED) | STORY-0081 |
| SCENARIO-0119 | log-only mode allows and logs an invalid grant (safe rollout) | integration | iteration | **laneq:** `cd /Users/ndn/development/laneq && uv run pytest tests/test_grpc_auth.py -k log_only` | STORY-0081 |
| SCENARIO-0120 | Grant key rotation via kid + int-timestamp interop | integration | iteration | **laneq:** `cd /Users/ndn/development/laneq && uv run pytest tests/test_auth.py -k rotation` + Go `go test ./grantauth/ -run IntTimestamps` | STORY-0079 |
