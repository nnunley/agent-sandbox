...
- AC-1b: **DONE (operator-authorized 2026-06-25) — live cluster on `enforce`.** External PR `selamy-labs/laneq#20` (from `nnunley/laneq:paseto-auth`; 4 CI gates green locally: ruff format/check, 186 pytest, coverage 96%). Auth-laneq deployed LIVE on `agent-host`: packaged `pyseto` 1.9.3 (`fleet-worker/pyseto.nix`, uv-build backend, not in nixpkgs), bumped `laneq.nix` → `paseto-auth` rev, declarative issuer pubkey, all via live `switch-to-configuration switch` (Temporal never bounced). **Rolled `log-only` → `enforce`:** grant-wired the Temporal worker's laneq client (`temporal/temporal_live_test.go` + `run-temporal-live.sh` attach grant+proof from container creds at `/srv/laneq/worker.{grant,key}`); provisioned the worker keypair (issuer-minted grant, worker private key on the container, issuer key on the Mac); verified on the LIVE cluster with a cross-compiled probe: in `log-only` the worker grant authenticated (no rejection log) while unauth was flagged, then in `enforce` the **authenticated worker → ALLOWED** and **unauthenticated → `UNAUTHENTICATED` "missing grant or proof metadata"**. No legitimate RPC rejected; `NRestarts=0`. **Follow-up (not blocking):** a Mac-side renewal helper (launchd/cron re-mint + re-push before the 720h grant `exp`) is still to build · impact:`journey` · seam:`e2e` · scenario:`SCENARIO-0119`
...
**Status:** **AC-1a done:ITER-0007c** (local cross-language real-wire log-only→enforce gate — T3 `cd9c61e`; issuer→`GrantSource` delivery via `laneq-grant` CLI — T4). **AC-1b DONE (operator-authorized 2026-06-25):** PR `selamy-labs/laneq#20` open; auth-laneq deployed LIVE on `agent-host` and rolled **`log-only` → `enforce`** with the Temporal worker grant-wired (`2db66bb`/`1c63f64`) — live cluster verified (auth ALLOWED, unauth `UNAUTHENTICATED`); Mac-side grant-renewal helper is the only follow-up. AC-2 = design note (held). gRPC-TLS and a pull/fetch broker RPC are deferred (Phase 2 / broker doc).
