...
**Execution command:** `cd modules/incus-dispatcher && go test . -run TestScenario0010_MacOffSPOF` (process-level e2e proof: Daemon with no operator input claims→runs→grades→escalates autonomously; state persists to durable file-backed stores; second Daemon instance reads same stores, proving AC-1/2/3)
