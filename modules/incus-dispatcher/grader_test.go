...
// TestRunGrade_ExcludesGeneratedArtifacts proves the grader applies only source
// hunks and skips generated artifacts (STORY-0068 AC-1 "source files wholesale"
// + AC-2 finding: the captured 13→0 diff carries a binary .lgb + generated.sums
// that cannot be patched and must be regenerated by `make generate`). The diff
// here touches a source file plus a "generated" file; with the generated path
// excluded, the apply succeeds and the source change lands.
func TestRunGrade_ExcludesGeneratedArtifacts(t *testing.T) {
	if _, err := exec.LookPath("git"); err != nil {
		t.Skip("git not available")
	}
	repo := t.TempDir()
	...
	git("init", "-q")
	os.WriteFile(filepath.Join(repo, "src.go"), []byte("package p\n\nconst V = 1\n"), 0644)
	os.WriteFile(filepath.Join(repo, "generated.sums"), []byte("OLD\n"), 0644)
	git("add", "-A")
	git("commit", "-qm", "base")

	// Worker diff changes both the source and the generated file.
	diff := []byte("diff --git a/src.go b/src.go\n--- a/src.go\n+++ b/src.go\n@@ -1,3 +1,3 @@\n package p\n \n-const V = 1\n+const V = 2\n" +
		"diff --git a/generated.sums b/generated.sums\n--- a/generated.sums\n+++ b/generated.sums\n@@ -1 +1 @@\n-OLD\n+NEW\n")

	gates := []GateSpec{
		{Name: GateGenerate, Cmd: []string{"sh", "-c", "true"}},
		{Name: GateClusterA, Cmd: []string{"sh", "-c", "grep -q 'V = 2' src.go && echo ok || (echo '--- FAIL: src'; exit 1)"}},
		{Name: GateCheckGenerated, Cmd: []string{"sh", "-c", "grep -q OLD generated.sums && echo ok || (echo '--- FAIL: gen-was-patched'; exit 1)"}},
		{Name: GateUntagged, Cmd: []string{"sh", "-c", "true"}},
		{Name: GateE2E, Cmd: []string{"sh", "-c", "true"}},
	}
	report, _, err := RunGrade(context.Background(), repo, diff, gates, "generated.sums")
	if err != nil {
		t.Fatalf("RunGrade: %v", err)
	}
	if !report.Passed {
		t.Errorf("expected pass: source applied, generated.sums left for regeneration; got %+v", report)
	}
}
